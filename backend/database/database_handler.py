...
class DatabaseHandler:
    ...
    def push(self, location_object):
        ...
        self.db.insert(self.__LOCATION_HISTORY, uniqueID=uID, username=username, longitude=longitude, latitude=latitude)
        self.db.insertReplace(self.__LOCATION, uniqueID=uID, username=username, longitude=longitude, latitude=latitude)
    ...
    def pull(self, usernames):
        ...
        for username in usernames:
            ...
            if uID:
                ...
                raise UserWarning("USER NOT FOUND!")
            result = self.db.get(self.__LOCATION, username=None, longitude=None, latitude=None, condition="username='{}' and uniqueID={}".format(username, uID))
            if result:
                location_objects.append({"username": result[0][0], "longitude": result[0][1], "latitude": result[0][2]})
